...
export default function BotonCompartir({
  ...
  url?: string;
  className?: string;
  /** Solo iconos, sin etiquetas. Para el encabezado en móvil. */
  compacto?: boolean;
}) {
  ...
  return (
    <div className={`flex flex-wrap gap-2 ${className}`}>
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-md bg-[#25D366] text-sm font-medium text-white transition hover:bg-[#1da851] ${
          compacto ? "px-3 py-2" : "px-4 py-2"
        }`}
      >
        <svg
          viewBox="0 0 24 24"
      ...
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white text-sm font-medium text-slate-700 transition hover:bg-slate-50 ${
          compacto ? "px-3 py-2" : "px-4 py-2"
        }`}
      >
        {compacto ? (
        ...
        {compacto ? (copiado ? "¡Listo!" : "") : copiado ? "¡Copiado!" : "Copiar enlace"}
      </button>
    </div>
  );
}
